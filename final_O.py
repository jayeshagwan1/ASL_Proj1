...
def main():
    ...
    while(cap.isOpened()):
        ...
        xyz, contours_dofh, hierarchy_dofh = cv2.findContours(
            thresh_dofh, 2, 1)
        ...
        print('\033[91m'+"The area of effective circle is " +
              '\033[0m', area_of_circle - area)
...
######################################################################################################################################
w = Label(
    root, text="Press the button below if you want to Listen to the instructions.")
w.config(width=75)
...
x = Label(
    root, text="Press the button below to run the application (Then,press s for sound).")
x.config(width=75)
